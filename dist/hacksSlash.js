...
export function generateRandomEquipment(playerLevel) {
    // Determine Rarity (simplified)
    let rarity;
    const randRarity = Math.random();
    if (randRarity < 0.05 && playerLevel > 20)
        rarity = 'Legendary'; // 5%
    else if (randRarity < 0.15 && playerLevel > 10)
        rarity = 'Epic'; // 10%
    else if (randRarity < 0.30 && playerLevel > 5)
        rarity = 'Rare'; // 15%
    ...
    switch (rarity) {
        case 'Legendary':
            adjectives = ADJECTIVES_LEGENDARY;
            break;
        case 'Epic':
            adjectives = ADJECTIVES_EPIC;
            break;
        case 'Rare':
            adjectives = ADJECTIVES_RARE;
            break;
        case 'Uncommon':
            adjectives = ADJECTIVES_UNCOMMON;
            break;
    ...
    for (let i = 0; i < numEffects; i++) {
        ...
        if (effectTypeRand < 0.4) { // Spirit Boost
            ...
            const handCondition = getRandomElement(['rock', 'paper', 'scissors']);
            effect = {
...
export function applyEquipmentEffects(playerData, baseSpiritGain, actionType, handPlayed) {
