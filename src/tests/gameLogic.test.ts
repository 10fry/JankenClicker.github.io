...
import { HandChoice, Outcome, PlayerData } from '../types';
...
function testPlayRound() {
  ...
  const playerChoice: HandChoice = 'rock';
  ...
  if(playerChoice === 'rock') assertEqual(playerData.stats.rocksThrown, 1, 'Rocks thrown should increment');
  ...
  (getOpponentChoice as any) = () => 'scissors'; // Force opponent to choose scissors for a win with rock
  ...
  (getOpponentChoice as any) = originalGetOpponentChoice; // Restore original function
}
