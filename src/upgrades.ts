import { UpgradesConfig, PlayerData, UpgradeConfigItem } from './types';

// Define UPGRADES_CONFIG
export const UPGRADES_CONFIG: UpgradesConfig = {
  spiritPerWin: {
    name: 'Spirit per Win',
    baseCost: 10,
    costMultiplier: 1.5, // Each level costs 1.5x the previous
    baseEffect: 1,       // Base +1 spirit per win
    description: (level: number, effect: number, cost: number) =>
      `Level ${level}: +${effect} Spirit per win. Cost: ${cost} Spirit.`,
  },
...
export function getUpgradeConfig(upgradeId: string): UpgradeConfigItem | undefined {
...
export function buyUpgrade(playerData: PlayerData, upgradeId: string): boolean {
  const config = getUpgradeConfig(upgradeId);
  if (!config) {
