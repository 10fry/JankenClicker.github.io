import { PlayerData, HandChoice, Outcome, EquipmentItem } from './types';
...
export let currentPlayer: PlayerData;

// DOM Element Variables
let rockButton: HTMLButtonElement | null = null;
let paperButton: HTMLButtonElement | null = null;
let scissorsButton: HTMLButtonElement | null = null;

let playerChoiceDisplay: HTMLElement | null = null;
let opponentChoiceDisplay: HTMLElement | null = null;
let outcomeDisplay: HTMLElement | null = null;
// let spiritGainedDisplay: HTMLElement | null = null; // Not explicitly in HTML, often part of outcome or log

let jankenSpiritDisplay: HTMLElement | null = null;
let totalWinsDisplay: HTMLElement | null = null;
let totalLossesDisplay: HTMLElement | null = null;
let totalDrawsDisplay: HTMLElement | null = null;


let upgradesContainer: HTMLElement | null = null;
// Specific upgrade elements will be handled dynamically

let manualSaveButton: HTMLButtonElement | null = null;
let resetDataButton: HTMLButtonElement | null = null;
let gameLogDisplay: HTMLElement | null = null;
let tenseiButton: HTMLButtonElement | null = null;
let tenseiTokensDisplay: HTMLElement | null = null;
let equipmentDisplay: HTMLElement | null = null;


function initializeUI() {
    rockButton = document.getElementById('rock-button') as HTMLButtonElement;
    paperButton = document.getElementById('paper-button') as HTMLButtonElement;
    scissorsButton = document.getElementById('scissors-button') as HTMLButtonElement;
    ...
    manualSaveButton = document.getElementById('save-button') as HTMLButtonElement;
    resetDataButton = document.getElementById('reset-button') as HTMLButtonElement;
    gameLogDisplay = document.getElementById('game-log-list');
    tenseiButton = document.getElementById('tensei-button') as HTMLButtonElement; // Added in HTML conceptually
    ...
    rockButton?.addEventListener('click', () => handleJankenPlay('rock'));
    paperButton?.addEventListener('click', () => handleJankenPlay('paper'));
    scissorsButton?.addEventListener('click', () => handleJankenPlay('scissors'));

    manualSaveButton?.addEventListener('click', handleManualSave);
    resetDataButton?.addEventListener('click', handleResetData);
    tenseiButton?.addEventListener('click', handleTensei);
...
function updateBattleResultDisplay(playerChoice: HandChoice, opponentChoice: HandChoice, outcome: Outcome, spiritGained: number) {
  ...
  if (outcomeDisplay) outcomeDisplay.textContent = `${outcome.toUpperCase()}! ${spiritGained >= 0 ? `+${spiritGained}` : spiritGained} Spirit.`;
}

function updateStatsDisplay() {
  if (totalWinsDisplay) totalWinsDisplay.textContent = currentPlayer.stats.totalWins.toString();
  if (totalLossesDisplay) totalLossesDisplay.textContent = currentPlayer.stats.totalLosses.toString();
  if (totalDrawsDisplay) totalDrawsDisplay.textContent = currentPlayer.stats.totalDraws.toString();
...
// --- Event Handlers ---
function handleJankenPlay(playerChoice: HandChoice) {
